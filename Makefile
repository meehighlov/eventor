...
.PHONY: db
db:
	docker run -d --name eventor-db -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=postgres -p 5432:5432 postgres:16

.PHONY: redis
redis:
	docker run -d --name eventor-redis -p 6379:6379 redis:latest
